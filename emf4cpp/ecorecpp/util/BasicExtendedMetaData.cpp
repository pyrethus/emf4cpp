...
// Mimics boost::is_any_of (for simplicity, only handles single char delimiters)
std::string is_any_of(const char delim) {
...
void split(std::vector<std::string>& tokens, const std::string& str, const std::string& delims) {
  ...
  std::string token;
  while (std::getline(iss, token, delims[0])) {
    if (!token.empty()) {
...
BasicExtendedMetaData::EStringList::ptr_type BasicExtendedMetaData::EDataTypeExtendedMetaData::basicGetEnumerationFacet() {
  mapping::EList<::ecore::EString>::ptr_type result(new ::ecorecpp::mapping::EListImpl<::ecore::EString>());
  ...
  if (eAnnotation->hasDetail("enumeration")) {
    ...
    split(tokens, enumerationLiteral, is_any_of(' '));
    for ( auto token : tokens ) {
      replace_all(token, "%20", " ");
      replace_all(token, "%25", "%");
...
void BasicExtendedMetaData::setEnumerationFacet(::ecore::EDataType_ptr eDataType,
  ...
  if (literals->size() == 0) {
    ...
    for ( auto literal : *literals ) {
      replace_all(literal, "%", "%25");
      replace_all(literal, " ", "%20");
      result += literal + ' ';
    }
